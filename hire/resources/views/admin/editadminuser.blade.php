    ...
    $("#validation").validationEngine({promptPosition : "topLeft", scroll: true});  
        ...
        <div class="row-form clearfix">
         ...
         ?>
          ...
          <div class="form-group subuser" @if($errors->has('state') || $getUserdetails->type==2) @else style="display:none" @endif >
            <label for="ogn">User Type</label>
          
                               <select class="form-control xyz" name="state">
                                <option value='0'>Select State</option>
                                @if(count($statelist)>0)
                                @foreach($statelist as $statelist)
                                <option value="{{$statelist->id}}" @if($statelist->id==$getUserdetails->assign_state) selected @endif>{{$statelist->state}}</option>
                                @endforeach()
                                @endif
